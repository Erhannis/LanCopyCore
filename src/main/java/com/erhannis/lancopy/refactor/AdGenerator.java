...
public class AdGenerator implements CSProcess {
    ...
    @Override
    public void run() {
        Advertisement ad = new Advertisement(dataOwner.ID, System.currentTimeMillis(), new ArrayList<>(), dataOwner.encrypted, dataOwner.encrypted ? dataOwner.tlsContext.sha256Fingerprint : null);
        try {
            ...
            while (true) {
                switch (alt.priSelect()) {
                    case 0: // commsIn
                        ...
                        ad = new Advertisement(dataOwner.ID, System.currentTimeMillis(), combined, dataOwner.encrypted, dataOwner.encrypted ? dataOwner.tlsContext.sha256Fingerprint : null);
                        adOut.write(ad);
                        break;
                }
            }
