...
public class TcpGetComm implements CSProcess {
    ...
    public class WsClient extends WebSocketListener {
        ...
        public void connect(TcpComm comm) {
            ...
            crew.startWrite();
            try {
                WebSocket ws = client.newWebSocket(new Request.Builder().url(new HttpUrl.Builder().scheme("http").host(comm.host).port(comm.port) + "ws/updates").build(), this);
                                    ...
                                    if (TcpComm.TYPE.equals(comm.type)) {
                                        ...
                                        if (highLatency) {
                                            ...
                                            attempts.add(new DThread(() -> {
                                                ...
                                                Request request = new Request.Builder().url(new HttpUrl.Builder().scheme("http").host(tc.host).port(tc.port).addPathSegments("get/data").build()).build();
                                                try {
                                            ...
                                            attempts.add(new DThread(() -> {
                                                ...
                                                Request request = new Request.Builder().url(new HttpUrl.Builder().scheme("http").host(tc.host).port(tc.port).addPathSegments("get/poke").build()).build();
                                                try {
                                                ...
                                                request = new Request.Builder().url(new HttpUrl.Builder().scheme("http").host(tc.host).port(tc.port).addPathSegments("get/data").build()).build();
                                                try {
                                                    Response response = dataOwner.ohClient.newCall(request).execute();
                        ...
                        for (Comm comm : comms) {
                            if (TcpComm.TYPE.equals(comm.type)) {
                                try {
                                    ...
                                    new ProcessManager(() -> {
                                        TcpComm tc = (TcpComm) comm;
                                        Request request = new Request.Builder().post(RequestBody.create(ladBytes, MediaType.get("lancopy/advertisement"))).url(new HttpUrl.Builder().scheme("http").host(tc.host).port(tc.port).addPathSegments("post/advertisement").build()).build();
                        ...
                        for (Comm comm : comms) {
                            if (TcpComm.TYPE.equals(comm.type)) {
                                try {
                                    new ProcessManager(() -> {
                                        TcpComm tc = (TcpComm) comm;
                                        Request request = new Request.Builder().get().url(new HttpUrl.Builder().scheme("http").host(tc.host).port(tc.port).addPathSegments("get/poke").build()).build();
                                        try (Response response = dataOwner.ohClient.newCall(request).execute()) {
                                            if (checkPoke) {
